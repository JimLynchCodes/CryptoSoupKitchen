...
contract CryptoSoupKitchen is VRFConsumerBase {
    ...
    uint256 public total_num_of_nft_tokens_to_give_away;
    ...
    // onlyOwner 
    {
        ...
        if (!nft_address_seen_before[nft_address]) {
            ...
            nft_collections.push(nft_address);

            nft_token_Ids_to_give_away.push(token_ids);

            nft_num_of_token_ids_to_give_away.push(token_ids.length);
        ...
        if (isJackpotWinner) {
           ...
           for (uint i=0; i<nft_collections_next_index; i++) { 
                ...
                if (tokenCount >= randomlyChosenNftTokenIndex) {
                    ...
                    uint256 tokenIndex = (randomness % nft_num_of_token_ids_to_give_away[i]) + 1;

                    // transfer nft
                    uint tokenId = nft_token_Ids_to_give_away[i][tokenIndex];
           
                    ERC721(nftAddress).transferFrom(address(this), requestedBy, tokenId);
                    ...
                    nft_token_Ids_to_give_away[i][tokenIndex] = oldLastTokenIdValue;
    ...
    function numOfTokensToGiveAwayForCollection(address nft_address)
        external
        view
        returns (uint256)
    {
        uint256 nft_index = nft_indices[nft_address];
        return nft_num_of_token_ids_to_give_away[nft_index];
    }
    
    function tokensToGiveAwayForCollection(address nft_address)
        external
        view
        returns (uint256[] memory)
    {
        uint256 nft_index = nft_indices[nft_address];
        return nft_token_Ids_to_give_away[nft_index];
    }
    ...
    function withdrawNativeTokenProfits() external 
    // onlyOwner 
    {
    ...
    function withdraw_all_native_token() public payable 
    // onlyOwner 
    {
        (bool success, ) = payable(msg.sender).call{
