...
contract CryptoSoupKitchen is VRFConsumerBase {
    ...
    function storeTokenIdsToGiveAwayForNft(
        ...
        // Make sure contract is the owner of the token_ids passed in.
        
        // WARNING: Unbounded loop in Solidity is kind of an anti-pattern 
        for (uint i=0; i<token_ids.length; i++) { 
            // tokenId owners must be this contract!
            address token_owner = ERC721(nftAddress).ownerOf(tokenId);
            require(token_owner == address(this), strConcat("Please transfer token ", tokenId, " to this contract:  ", address(this)));
        }
    ...
    function withdrawNativeTokenProfits() external isOwner {
